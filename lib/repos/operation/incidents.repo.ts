...
export const incidentsRepo = {
  ...
  async assignForUser(user: User, id: string, assignedToUserId: string): Promise<IncidentEntity | null> {
    if (isDbMode()) {
      const res = await fetch(`/api/v1/operation/incidents/${id}`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
        credentials: 'include',
        body: JSON.stringify({ status: 'ASSIGNED', assignedToUserId }),
      });
      if (res.status === 404) return null;
      const data = (await res.json().catch(() => null)) as { incident?: IncidentEntity | null; error?: string } | null;
      if (!res.ok) throw new Error(data?.error || 'No autorizado');
    ...
    if (!assignee) throw new Error('Selecciona un responsable valido para este edificio.');
