...
export async function PATCH(req: Request, ctx: { params: Promise<{ id: string }> }) {
  ...
  if (action === 'CANCEL') {
    ...
    if (current.status === 'REJECTED') return NextResponse.json({ error: 'No puedes cancelar una reserva rechazada.' }, { status: 400 });
    if (new Date(current.end_at).getTime() < Date.now()) {
      return NextResponse.json({ error: 'No puedes cancelar una reserva que ya finalizó.' }, { status: 400 });
    }
