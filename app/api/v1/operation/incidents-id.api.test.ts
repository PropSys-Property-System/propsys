...
describe('operation incidents [id] API (audit hardening)', () => {
  ...
  it('updates incident status and records audit log (BUILDING_ADMIN closes incident)', async () => {
    poolQuery.mockReset();
    clientQuery.mockReset();
    (sessionUser as unknown as { role: string }).role = 'MANAGER';
    (sessionUser as unknown as { internalRole: string }).internalRole = 'BUILDING_ADMIN';
    ...
    clientQuery.mockImplementation(async (sql: string, params?: unknown[]) => {
      ...
      if (sql.includes('UPDATE incidents')) return { rows: [{ ...current, status: 'CLOSED' }] };
    ...
    const req = new Request('http://localhost/api/v1/operation/incidents/inc_2', {
      method: 'PATCH',
      headers: { 'content-type': 'application/json' },
      body: JSON.stringify({ status: 'CLOSED' }),
    });
    const res = await patchIncident(req, { params: Promise.resolve({ id: 'inc_2' }) });
    expect(res.status).toBe(200);
    const data = (await res.json()) as { incident?: { status: string } };
    expect(data.incident?.status).toBe('CLOSED');
