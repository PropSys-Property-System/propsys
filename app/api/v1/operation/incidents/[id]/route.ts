...
export async function PATCH(req: Request, ctx: { params: Promise<{ id: string }> }) {
  ...
  const status =
    body?.status === 'REPORTED' || body?.status === 'ASSIGNED' || body?.status === 'IN_PROGRESS' || body?.status === 'RESOLVED' || body?.status === 'CLOSED'
      ? body.status
      : null;
  const assignedToUserId = typeof body?.assignedToUserId === 'string' && body.assignedToUserId.trim() ? body.assignedToUserId.trim() : null;
  ...
  if (!canAccessTenantEntity(user, current.client_id)) return NextResponse.json({ incident: null }, { status: 404 });
  ...
  if (assignedToUserId) {
    ...
    const assignee = await pool.query<{ ok: boolean }>(
      `SELECT true as ok
       FROM users u
  ...
  try {
    const updated = await withTransaction(pool, async (db) => {
      const res = await db.query<{
        ...
        [id, status, now, assignedToUserId]
      );
      ...
      await insertAuditLog(db, {
        ...
        entity: 'Incident',
        entityId: id,
        metadata: { fromStatus: current.status, toStatus: status, assignedToUserId },
        oldData: current,
        newData: res.rows[0],
      });
      return res;
