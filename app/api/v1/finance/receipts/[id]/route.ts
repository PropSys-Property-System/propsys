...
async function hasPaymentProof(pool: ReturnType<typeof getPool>, receiptId: string) {
...
export async function PATCH(req: Request, ctx: { params: Promise<{ id: string }> }) {
  ...
  if (current.status !== 'PENDING' && current.status !== 'OVERDUE') {
    return NextResponse.json({ error: `No se puede modificar un recibo que está ${labelReceiptStatus(current.status as Receipt['status'])}.` }, { status: 400 });
  }
...
export async function PUT(req: Request, ctx: { params: Promise<{ id: string }> }) {
  ...
  if (await hasPaymentProof(pool, id)) {
    return NextResponse.json({ error: 'No se puede editar un recibo que ya tiene un comprobante de pago cargado.' }, { status: 400 });
  }
